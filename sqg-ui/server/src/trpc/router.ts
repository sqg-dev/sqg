...
export const appRouter = t.router({
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        for (const t of result.tables) {
          annotations.push({ id: t.id, type: 'TABLE', line: t.line, one: false, pluck: false, sql: '' });
        }
  ...
  listExamples: t.procedure.query(() => {
    ...
    const roots = [
      ...
      resolve(dirname(fileURLToPath(import.meta.url)), '../../../..'), // trpc/ -> src/ -> server/ -> sqg-ui/ -> repo root
