<script lang="ts">
  ...
  import type { SqgQuery } from '@sqg-ui/shared';
...
<div class="h-full flex flex-col bg-gray-900">
  ...
  <div class="flex-1 overflow-auto">
    ...
    {:else if projectState.hasProject}
      ...
      <!-- Migrations Section -->
      {@const displayMigrations = liveMigrations ?? projectState.migrations.map(m => ({ id: m.id, type: 'MIGRATE' as const, one: false, pluck: false, line: m.line }))}
          ...
          <div class="pb-2">
            {#each displayMigrations as item}
              <button
                ...
                title="{tabsState.activeTab?.fileName ?? ''}:{item.line}"
                class="w-full px-4 py-1.5 flex items-center gap-2 text-left hover:bg-gray-800/50 transition-colors"
              >
      ...
      <!-- Queries Section -->
      {@const displayQueries = liveQueries ?? projectState.queryGroups.queries.map(q => ({ id: q.id, type: q.type, one: q.modifiers.one, pluck: q.modifiers.pluck, line: q.line }))}
          ...
          <div class="pb-2">
            {#each displayQueries as query}
              <button
                ...
                title="{tabsState.activeTab?.fileName ?? ''}:{query.line}"
      ...
      <!-- Exec Section -->
      {@const displayExecs = liveExecs ?? projectState.queryGroups.execs.map(q => ({ id: q.id, type: q.type, one: false, pluck: false, line: q.line }))}
