    ...
    for (const sql of project.sql) {
      ...
      for (const sqlFile of sql.files) {
        ...
        for (const [engine, gens] of gensByEngine) {
          ...
          // Set expected query count on reporter for spinner
          const reporterAny = reporter as any;
          if (reporterAny?.setQueryTotal) {
          ...
          try {
            ...
            if (tables.length > 0) {
              await dbEngine.introspectTables(tables, reporter);
            }
          ...
          ui?.succeedPhase(
            `Generated ${gens.length} ${gens.length === 1 ? "file" : "files"} from ${sqlFile}`,
          );
        }
      }
    }
