...
export class JavaGenerator extends BaseGenerator {
  ...
  async beforeGenerate(
    ...
    // Whether to emit the optional instrumentation seam (observer field,
    // constructor overload, per-operation start/end callbacks). Opt-in via
    // `config.observer: true`; off by default so existing projects see no diff.
  ...
  override auxiliaryFiles(gen: GeneratorConfig): AuxiliaryFile[] {
  ...
  async afterGenerate(outputPath: string): Promise<void> {
    try {
      ...
      const [prettierModule, javaModule] = await Promise.all([
        import("prettier/standalone"),
        import("prettier-plugin-java"),
      ]);
      const prettier = interopDefault(prettierModule);
      const prettierPluginJava = interopDefault(javaModule) as unknown as Plugin;
