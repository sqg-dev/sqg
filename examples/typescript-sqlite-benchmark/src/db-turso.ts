...
CREATE INDEX IF NOT EXISTS idx_posts_published ON posts(published);`,
  ...
  async countUsers(): Promise<number | undefined> {
    const stmt = await this.prepare(
      "countUsers",
...
WHERE p.id =?;`,
  ...
  async countPostsByUser(userId: number): Promise<number | undefined> {
    const stmt = await this.prepare(
      "countPostsByUser",
      "SELECT COUNT(*) FROM posts WHERE user_id =?;",
    );
    const row = (await stmt.get(userId)) as Record<string, number> | undefined;
    return row ? Object.values(row)[0] : undefined;
  }
