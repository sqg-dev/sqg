...
**Postgres sources (DuckDB):** Declare `sources: [{ name: prod, type: postgres }]` in `sqg.yaml` to query an attached Postgres database (`prod.public.…`). Tag the schema with `-- BASELINE <name> :source=prod` (valid Postgres DDL). At generation SQG starts a Postgres testcontainer (needs Docker — fails fast with a clear message if unavailable), applies the `:source=` blocks natively (real PG types), attaches it into DuckDB, and type-checks — none of it is emitted. For runtime, SQG generates an `attach<Source>(connectionString)` helper that the app calls to attach the real database under the same alias; the helper runs `INSTALL postgres; LOAD postgres;` (idempotent) before `ATTACH`. Requires a `duckdb` generator.

- **No Docker / no drift:** set `url` on a postgres source (`{ name: prod, type: postgres, url: $DATABASE_URL }`) to introspect against an existing database instead of a container. `url` may be a literal DSN or `$VAR`/`${VAR}` read from the environment. With `url` set, no container starts and the live schema is used (the `:source=` BASELINE blocks are ignored).
- **Scoping:** sources are declared once at the project top level but only affect the `sql` blocks whose queries reference them (a `:source=<name>` BASELINE or a `<name>.` catalog reference). Unrelated generators stay byte-identical — no `attach<Source>` helper, no container started — when a source used by another block is added.

Sources without `type` are **file sources**: a path exposed as an inlined `${sources_<name>}` variable for `QUERY`/`EXEC` (e.g. `read_csv`, sqlite `ATTACH`). The variable name is `sources_` + `name`, or the file basename (sans extension) when `name` is omitted — prefer an explicit `name:`. **Runtime contract (identical across TS/Java/Python):** `${sources_<name>}` becomes a runtime string method parameter interpolated into the SQL; the generation-time absolute path is used only for introspection and is never baked into generated code.

The `sql[]` block schema is **strict**: unknown keys (e.g. a misplaced `sources:` under an sql block — it belongs at the top level) are rejected with a pointed `CONFIG_VALIDATION_ERROR` rather than silently dropped.
