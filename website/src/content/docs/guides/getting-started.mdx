...
The fastest way to get started is using the `sqg init` command, which launches an **interactive wizard**:

```bash
sqg init
```

The wizard walks you through choosing a generator, project name, and output directory:
...
You can also run non-interactively by specifying `--generator`:
...
4. **Use the generated code**
   ...
   // Apply migrations (creates tracking table, applies in order)
...
|-----------|-------------|---------------|
| `typescript/sqlite` | TypeScript for SQLite (sync API) | [TypeScript + SQLite](/generators/typescript-sqlite/) |
| `typescript/duckdb` | TypeScript for DuckDB (async API) | [TypeScript + DuckDB](/generators/typescript-duckdb/) |
| `java/sqlite` | Java with JDBC for SQLite | [Java + JDBC](/generators/java-jdbc/) |
| `java/duckdb` | Java with JDBC for DuckDB | [Java + JDBC](/generators/java-jdbc/) |
| `java/duckdb/arrow` | Java with Arrow API (high-performance) | [Java + DuckDB Arrow](/generators/java-duckdb-arrow/) |
| `java/postgres` | Java with JDBC for PostgreSQL | [Java + JDBC](/generators/java-jdbc/) |
| `python/sqlite` | Python for SQLite (sqlite3) | [Python](/generators/python/) |
| `python/duckdb` | Python for DuckDB | [Python](/generators/python/) |
| `python/postgres` | Python for PostgreSQL (psycopg3) | [Python](/generators/python/) |

See individual generator documentation for configuration options, type mappings, and usage examples.

## Database Engines

| Engine | Description | TypeScript | Java | Python |
|--------|-------------|------------|------|--------|
| `sqlite` | Embedded, no server required | [typescript/sqlite](/generators/typescript-sqlite/) | [java/sqlite](/generators/java-jdbc/) | [python/sqlite](/generators/python/) |
| `duckdb` | Analytics, complex types | [typescript/duckdb](/generators/typescript-duckdb/) | [java/duckdb](/generators/java-jdbc/), [java/duckdb/arrow](/generators/java-duckdb-arrow/) | [python/duckdb](/generators/python/) |
| `postgres` | Full-featured SQL server | — | [java/postgres](/generators/java-jdbc/) | [python/postgres](/generators/python/) |

**Notes:**
- SQLite and DuckDB use in-memory databases for type introspection (no server needed)
- PostgreSQL requires a running server; set `SQG_POSTGRES_URL` environment variable, or SQG will auto-start a container via [Testcontainers](https://testcontainers.com/)
...
- **Generators:**
  ...
  - [Python](/generators/python/) - SQLite, DuckDB, and PostgreSQL with frozen dataclasses
