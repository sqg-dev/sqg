...
SQG (SQL Query Generator) is a build-time tool that generates type-safe database access code from annotated SQL files. You write SQL queries with simple comment annotations, and SQG generates TypeScript, Java, or Python code with full type safety.
...
Currently supported:
- **SQLite** - via better-sqlite3, node:sqlite, libSQL, or Turso
- **DuckDB** - via @duckdb/node-api
- **PostgreSQL** - via pg (with ENUM, array, and TIMESTAMPTZ support)
...
- **Python** - for SQLite (sqlite3), DuckDB, and PostgreSQL (psycopg3) with frozen dataclasses
...
sqg init --generator typescript/sqlite --output ./src/db
```
...
**Recommended: Use the built-in migration runner.** Enable it with `config.migrations: true` in your `sqg.yaml`:
...
This generates a static `applyMigrations()` method that handles everything:
...
// Tracks applied migrations in _sqg_migrations table
// Applies new ones in order, inside a transaction
MyApp.applyMigrations(db);
```

The tracking table is scoped by project name (from `sqg.yaml`), so **multiple sqg projects can share the same database**.

**Alternative: Manual migration management.** SQG also generates a `getMigrations()` method that returns raw SQL strings if you prefer to use external migration tools:
...
```json
{
  ...
  "error": {
    ...
    "message": "Invalid generator 'typescript/sqlite/invalid'. Valid generators: typescript/sqlite, typescript/duckdb, java/sqlite, java/duckdb, java/duckdb/arrow, java/postgres",
    "suggestion": "Use 'typescript/sqlite' instead"
  }
}
```
